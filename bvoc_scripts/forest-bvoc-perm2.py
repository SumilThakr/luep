...
# Read PFT.nc, specifically the trees (PFTs 1-8)
...
forestFrac = pft1[:] + pft2[:] + pft3[:] + pft4[:] + pft5[:] + pft6[:] + pft7[:] + pft8[:]

# ISOP
isopResult = ((600.0 * pft1[:]) + (3000.0 * pft2[:]) + (1.0 * pft3[:]) + (7000.0 * pft4[:]) + (10000.0 * pft5[:]) + (7000.0 * pft6[:]) + (10000.0 * pft7[:]) + (11000.0 * pft8[:])) * isopf * conversion / forestFrac
isopResult.set_fill_value(0.0)
isopResult = isopResult.filled(fill_value=0.0)

# MTPA
mtpaResult = ((1130.0 * pft1[:]) + (1130.0 * pft2[:]) + (960.0 * pft3[:]) + (920.0 * pft4[:]) + (690.0 * pft5[:]) + (920.0 * pft6[:]) + (690.0 * pft7[:]) + (690.0 * pft8[:])) * mtpaf * conversion / forestFrac
mtpaResult.set_fill_value(0.0)
mtpaResult = mtpaResult.filled(fill_value=0.0)

# MTPO
mtpoResult = ((180.0 * pft1[:]) + (180.0 * pft2[:]) + (170.0 * pft3[:]) + (150.0 * pft4[:]) + (150.0 * pft5[:]) + (150.0 * pft6[:]) + (150.0 * pft7[:]) + (150.0 * pft8[:])) * mtpaf * conversion / forestFrac
mtpoResult.set_fill_value(0.0)
mtpoResult = mtpoResult.filled(fill_value=0.0)

result = isopResult + mtpaResult + mtpoResult

out = netCDF4.Dataset('forest-bvoc.nc', 'w', format='NETCDF3_64BIT_OFFSET', clobber=True)
...
if len(out.dimensions) == 0:
    ...
    pm_out.long_name = "bVOC emissions from trees"
    pm_out[:] = 0.0
