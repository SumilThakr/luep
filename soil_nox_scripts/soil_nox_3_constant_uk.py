def run(inputdir, scenario_name=None, n_application_path=None):
    """
    Modified soil_nox_3_constant.py for UK scenarios with scenario-specific nitrogen application
    
    Args:
        inputdir: Base input directory path
        scenario_name: UK scenario name (e.g., 'extensification_current_practices')
        n_application_path: Path to scenario-specific N application file (NetCDF or TIF)
    """
    ...
    if scenario_name:
        wdir = f"./intermediate/scenario_{scenario_name}"
        os.makedirs(wdir, exist_ok=True)
        print(f"Processing soil NOx for UK scenario: {scenario_name}")
        print(f"Working directory: {wdir}")
    else:
        wdir = "./"
        print("Processing soil NOx with global data")
    ...
    ph_raster      = [(os.path.join(inputdir, 'T_PH_H2O.tiff'),1)]
    ph_raster_out       = os.path.join(wdir, 'ph_effect.tif')
    ...
    lu_raster      = [(os.path.join(inputdir, 'gblulcg20_10000.tif'),1)]
    lu_raster_out       = os.path.join(wdir,'lu_effect.tif')
    ...
    soc_raster           = [(os.path.join(inputdir, 'T_OC.tiff'),1)]
    soc_raster_out       = os.path.join(wdir,'soc_effect.tif')
    def kSOC(SOC):
        k = 0.0
    ...
    clim_raster      = [(os.path.join(inputdir, 'Beck_KG_V1_present_0p5.tif'),1)]
    clim_raster_out       = os.path.join(wdir, 'clim_effect.tif')
    ...
    t0_raster_out       = os.path.join(wdir, 't0_effect.tif')
    ...
    ############################################################
    # 5         Nitrogen fertilization (UK SCENARIO SPECIFIC)
    ############################################################
    
    def convert_netcdf_to_tif(netcdf_path, tif_path, variable_name='N_application'):
        """
        Convert UK scenario N application NetCDF to TIF format for pygeoprocessing
        """
        print(f"Converting NetCDF to TIF: {netcdf_path} -> {tif_path}")
        
        # Read NetCDF file
        with nc.Dataset(netcdf_path, 'r') as ncfile:
            # Get data and coordinates
            n_data = ncfile.variables[variable_name][:]
            lat = ncfile.variables['lat'][:]
            lon = ncfile.variables['lon'][:]
            
            # Get geotransform from coordinate arrays
            ...
            geotransform = [
                lon[0] - lon_res/2,  # top-left x
                lon_res,             # pixel width  
                0,                   # rotation (0)
                lat[0] + lat_res/2,  # top-left y
                0,                   # rotation (0) 
                -lat_res             # pixel height (negative)
            ]
            
            # Create TIF file
            driver = gdal.GetDriverByName('GTiff')
            rows, cols = n_data.shape
    ...
    # Determine nitrogen application data source
    if n_application_path and scenario_name:
        # UK scenario mode - use scenario-specific data
        print(f"Using UK scenario nitrogen data: {n_application_path}")
        
        # Convert NetCDF to TIF if needed
        if n_application_path.endswith('.nc'):
            n_tif_path = os.path.join(wdir, f'{scenario_name}_n_application.tif')
            convert_netcdf_to_tif(n_application_path, n_tif_path)
            n_raster_input = [(n_tif_path, 1)]
        else:
            # Already a TIF file
            n_raster_input = [(n_application_path, 1)]
    else:
        # Global mode - use existing totN.tif
        print("Using global nitrogen data: totN.tif")
        n_raster_input = [(os.path.join(inputdir, 'totN.tif'), 1)]

    n_raster_out = os.path.join(wdir, 'n_effect.tif')

    def kN(N_rate):
        """
        Nitrogen effect calculation from Yan et al. (2005)
        
        N_eff = 0.03545 (coefficient)
        122/365 = temporal factor (growing season)
        """
        return 1.0 + (0.03545 * N_rate * 122.0/365.0)

    kN_v = np.vectorize(kN)

    geop.raster_calculator(base_raster_path_band_const_list=n_raster_input,
                                       local_op=kN_v, 
    ...
    print(f"Nitrogen effect calculated and saved to: {n_raster_out}")
    ...
    lai_raster          = [(os.path.join(inputdir, 'LAI', 'out_sum.tiff'),1)]
    crf_raster_out       = os.path.join(wdir, 'crf_effect.tif')
    ...
    ############################################################
    # This is done in the next script, soil_nox_5_align_uk.py
    
    print(f"Soil NOx constant effects calculated for scenario: {scenario_name or 'global'}")
    print(f"Intermediate files saved to: {wdir}")