#!/usr/bin/env python3
"""
Run bVOC emissions processing for UK scenarios with PROPER folder organization

Saves results in: outputs/uk_results/scenario_name/bvoc_emissions.nc
"""
...
def run_bvoc_processing():
    """Run bVOC emissions processing"""
    
    print(f"  📊 Running bVOC emissions processing...")
    
    cmd = ["/Users/sumilthakrar/yes/envs/luep-analysis/bin/python", "run_bvoc_emissions.py"]
    ...
    if outputs_path.exists():
        for file_path in outputs_path.glob("*"):
            if file_path.is_file() and file_path.name != "txt":
                if "bvoc" in file_path.name:
                    # Rename to proper format: bvoc_emissions.nc (no scenario prefix)
                    if file_path.suffix == ".nc":
                        new_name = "bvoc_emissions.nc"
                    elif file_path.suffix == ".tif":
                        new_name = "bvoc_emissions.tif"
                    else:
                        continue
...
def main():
    """Test proper folder organization"""
    
    print("🧪 TESTING PROPER UK FOLDER ORGANIZATION")
    print("=" * 50)
    print("Structure: outputs/uk_results/scenario_name/bvoc_emissions.nc")
    ...
    for i, scenario in enumerate(UK_SCENARIOS, 1):
        ...
        # Run bVOC processing
        success, duration = run_bvoc_processing()
