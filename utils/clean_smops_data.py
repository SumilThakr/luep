#!/usr/bin/env python3
"""
SMOPS Data Cleaning Utility

This script removes unused variables from SMOPS NetCDF files to save storage space.
Based on analysis of the land use emissions processor (LUEP) codebase, only one
variable is actually used:
- Blended_SM: Blended Soil Moisture (used in dust emissions and soil NOx models)

The original SMOPS files contain ~30+ variables including individual sensor data
(SMOS, ASCAT_A, ASCAT_B, AMSR2, GMI, SMAP, etc.), timing information, and QA flags.
This script keeps only the required variable plus coordinate dimensions.

The cleaned files will be dramatically smaller since we're keeping only 1 out of 30+ variables.

Usage:
    python utils/clean_smops_data.py <input_directory> <output_directory>
    python utils/clean_smops_data.py inputs/SMOPS inputs/SMOPS_cleaned

Requirements:
    - netCDF4 library
    - Original SMOPS files in NetCDF format (NPR_SMOPS_CMAP_D*.nc)
...
# Required SMOPS variables for LUEP (only Blended_SM is used)
REQUIRED_VARS = ['Blended_SM']
...
def clean_file_with_nco(input_file, output_file):
    """Clean SMOPS file using NCO tools for efficient processing."""
    try:
...
def clean_file_with_python(input_file, output_file):
    """Clean SMOPS file using Python netCDF4 library (fallback method)."""
    try:
        ...
        with Dataset(input_file, 'r') as src:
            ...
            with Dataset(output_file, 'w', format='NETCDF4') as dst:
                ...
                # Copy coordinate variables (Longitude, Latitude)
                coord_vars = []
                for name, variable in src.variables.items():
                    if name in src.dimensions or name.lower() in ['longitude', 'latitude']:
...
def clean_smops_directory(input_dir, output_dir, use_nco=True):
    """Clean all SMOPS files in a directory."""
    ...
    # Find all SMOPS NetCDF files (pattern: NPR_SMOPS_CMAP_D*.nc)
    nc_files = list(input_path.glob("NPR_SMOPS_CMAP_D*.nc"))
    
    if not nc_files:
        logger.error(f"No SMOPS files found in {input_dir}")
        logger.info("Looking for files matching pattern: NPR_SMOPS_CMAP_D*.nc")
        return False
    
    logger.info(f"Found {len(nc_files)} SMOPS files to process")
    ...
    for nc_file in nc_files:
        ...
        if successful_files == 0:
            ...
            logger.info(f"Original variables ({len(variables)}): {variables[:10]}...")  # Show first 10
            logger.info(f"Will keep only: {REQUIRED_VARS}")
            
            # Calculate expected reduction
            expected_reduction = ((len(variables) - len(REQUIRED_VARS)) / len(variables)) * 100
            logger.info(f"Expected size reduction: ~{expected_reduction:.0f}%")
    ...
    if successful_files > 0:
        ...
        if successful_files < len(nc_files):
            remaining_files = len(nc_files) - successful_files
            avg_original_size = total_size_before / successful_files if successful_files > 0 else 0
            avg_cleaned_size = total_size_after / successful_files if successful_files > 0 else 0
            potential_additional_savings = (avg_original_size - avg_cleaned_size) * remaining_files
            logger.info(f"Potential additional savings from remaining files: ~{potential_additional_savings:.1f}MB")
...
def main():
    """Main function."""
    if len(sys.argv) != 3:
        print("Usage: python utils/clean_smops_data.py <input_directory> <output_directory>")
        print("\nExample:")
        print("  python utils/clean_smops_data.py inputs/SMOPS inputs/SMOPS_cleaned")
        print("\nThis script looks for files matching pattern: NPR_SMOPS_CMAP_D*.nc")
        sys.exit(1)
    
    input_dir = sys.argv[1]
    output_dir = sys.argv[2]
    
    logger.info("SMOPS Data Cleaning Utility")
    logger.info(f"Input directory: {input_dir}")
    logger.info(f"Output directory: {output_dir}")
    logger.info(f"Required variables: {REQUIRED_VARS}")
    logger.info("File pattern: NPR_SMOPS_CMAP_D*.nc")
    ...
    # Clean the directory
    success = clean_smops_directory(input_dir, output_dir, use_nco)
    ...
    logger.info("Note: SMOPS files should show dramatic size reduction since we're keeping only 1 out of 30+ variables")
